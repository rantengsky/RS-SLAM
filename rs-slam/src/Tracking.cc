...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Map *pMap, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor):
    mState(NO_IMAGES_YET), mSensor(sensor), mbOnlyTracking(false), mbNewSegImgFlag(false), mbVO(false), mpORBVocabulary(pVoc),
...
void Tracking::GrabImageSem(const cv::Mat &imSemL, const cv::Mat &imSemC, const cv::Mat &imSemConf)
{
//    std::unique_lock <std::mutex> lock(mMutexNewImgSegment);
    cout << "--Tracking::GrabImageSem()语义图像赋值--" << endl;
    imSemL.copyTo(mImS_L);//将订阅的分割图像进行copy给mImS_L全局变量
    imSemC.copyTo(mImS_C);//将订阅的分割图像进行copy给mImS_C全局变量
    imSemConf.copyTo(mImS_Conf);//将订阅的分割图像进行copy给mImS_Conf全局变量
...
// 2. 光流法检测关键点=====
    ...
    flag = TrackHomo(homo);
    std::chrono::steady_clock::time_point start_Flow = std::chrono::steady_clock::now();
    if(flag && !homo.empty() )
        mFlow.ComputeMask(mImGray, homo, mImMask, BTh);// 剔除相机运动 根据光流计算 动/静点 mask
    else
        mFlow.ComputeMask(mImGray,mImMask, BTh);// 根据光流计算 动/静点 mask
    std::chrono::steady_clock::time_point end_Flow = std::chrono::steady_clock::now();
    double Flow_consume= std::chrono::duration_cast<std::chrono::duration<double> >(end_Flow - start_Flow).count();
    cout << "wait for new Flow time  = " << Flow_consume*1000 << endl;
// ===== */
...
//    }
    ...
    // 去除 imMask 内的关键点=====================
    mimLabel = mImS_L;//将mImS赋值用于FrameDrawer的显示

    //使用光流和语义分割剔除动点
//    mCurrentFrame = Frame(mImGray,mImDepth,
//                          mimMask, // 构造帧的时候，仅仅保留 剔除mask为0 处的 关键点
//                          mImS_L, //语义分割标签
//                          timestamp,mpORBextractorLeft,
//                          mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);

    //使用光流和语义分割和分割判断（根据分割结果判断潜在移动的静态物体）剔除动点
    std::chrono::steady_clock::time_point start_mMoment = std::chrono::steady_clock::now();
    dynamic_chair = mMoment.ComputeCentralMoment(mImS_C, mImS_L);
    std::chrono::steady_clock::time_point end_mMoment = std::chrono::steady_clock::now();
    double mMoment_consume= std::chrono::duration_cast<std::chrono::duration<double> >(end_mMoment - start_mMoment).count();
    cout << "wait for new Moment time  =" << mMoment_consume*1000 << endl;
    mCurrentFrame = Frame(mImGray, mImDepth,
                          mImMask, // 构造帧的时候，仅仅保留 剔除mask为0 处的 关键点
                          mImS_L, //语义分割标签
                          dynamic_chair,//潜在移动的椅子的中心点
                          timestamp, mpORBextractorLeft,
                          mpORBVocabulary, mK, mDistCoef, mbf, mThDepth);
...
// -------------【4】跟踪-------------------
    Track();// 跟踪后 就能够得到 位姿
    mImTcw = mCurrentFrame.mTcw.clone();
// -------------【5】返回相机运动
    return mImTcw;
}
...
void Tracking::CreateNewKeyFrame()
{
    ...
    if(mSensor == System::RGBD)// rgbd
    {
        pKF = new KeyFrame(mCurrentFrame,mpMap,mpKeyFrameDB,
                           this->mImRGB, this->mImDepth, this->mImS_L, this->mImS_C, this->mImS_Conf, this->mImMask, this->mImTcw);}
    else
        pKF = new KeyFrame(mCurrentFrame,mpMap,mpKeyFrameDB);
...
//    mpMapDrawer->insertKeyFrame( pKF,this->mImS_C,this->mImS_L ,this->mImRGB, this->mImDepth );
